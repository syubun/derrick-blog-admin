...
<script>
  ...
  import { putSerie } from '@/api/serie.js'

  import mybus from '@/utils/mybus.js'
  import { difference, without, map, sortBy } from 'lodash-es'
  ...
  export default defineComponent({
    setup() {
      ...
      const onSubmit = () => {
        validate().then(async () => {
          await putSerie(unref(id), toRaw(modelRef))
          message.success('修改系列成功')
          initVditor()
          // 重置
          resetFields()
          // 關閉 add model
          mybus.emit('serieEdit')
        })
      }
      ...
      // md 編輯器
      const initVditor = () => {
        contentEditor.value = new Vditor('seriesEdit', {
          height: 360,
          lang: 'zh_TW',
          input: (value) =>
      ...
      mybus.on('serieEditDate', ({ val }) => {
        id.value = val.id
        modelRef.name = val.name
        modelRef.description = val.description
        modelRef.categorieId = val.categorieId
        modelRef.article = map(sortBy(val.article, 'pivot.page'), 'id')
        initVditor()
      })
      return {
        labelCol: {
